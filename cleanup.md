...
>
> **Edit only the listed files.** Read anything in the repo you need to — you'll have to, to prove something is unreferenced — but confine edits to the list. The one exception is the extraction pass, which may create files inside the repo's existing layout.
>
...
**Move a symbol only when every reference to it is inside the listed files** — grep first. If anything outside the list imports it, leave it, or re-export from the original path so no unlisted file has to change.
