...
1. Default branch: `git symbolic-ref refs/remotes/origin/HEAD --short | sed 's@^origin/@@'`, falling back to `main` then `master`.
2. Changed files: `git diff --name-only <default>...HEAD`, plus anything uncommitted from `git status --porcelain`.
3. If the user passed paths as arguments, use those instead. The carve-outs still apply.
4. **Carve out**, even when the diff touches them:
   ...
   - Migrations — an append-only historical record, not code to tidy
   - Anything marked `WIP`, `LOCAL-ONLY`, or `DO NOT COMMIT`
5. Empty list → print "nothing on this branch to clean" and stop.
6. Print the scope. Over 25 files, summarize by area and confirm before dispatching.

## 2. Establish the check

Derive the typecheck or lint command from the repo — `package.json` scripts, the lockfile, `pyproject.toml`, `Makefile`, `Cargo.toml`, or the CI workflow. Run it **before pass 1**: if the tree is already broken, say so and ask whether to continue, so no pass gets blamed for a pre-existing failure.

Re-run it after each pass, scoped to the touched files. A pass that breaks the tree ends the run — report and stop, don't dispatch the next one.
...
>
> **Behavior stays identical.** Every code path, return value, response shape, error message, log line, raised exception and side-effect order is what it was before you touched the file. If you can't convince yourself a change is behavior-preserving, it isn't — leave it.
>
> **Edit only the listed files.** Read anything in the repo you need to — you'll have to, to prove something is unreferenced — but confine edits to the list.
>
...
>
> Report back: files edited with LOC deltas, the highest-value changes as `file:line` plus one line each on why, what you left alone and why, and bugs noticed but not fixed. Brief — four of these get aggregated into one report.

### Pass 1 — Dead code

Remove what's verifiably unreferenced: unused functions, classes, exports, imports, props, parameters, type fields; unreachable branches and constant-false guards; TODOs whose work is done; comments describing code that no longer exists; vestigial naming left behind by a migration.

- ❌ Deleting an exported handler because nothing imports it — it's registered by name in a route table.
- ❌ Deleting an unused parameter on one implementation of an interface.
- ✅ Deleting an import with zero references anywhere in the repo.
- ✅ Deleting a private helper whose only caller was removed earlier in this same branch.

Leave: public API surface, documented caller-parity params, anything out-of-scope files import.

### Pass 2 — Simplification

Inline duplication where a shared util **already exists**, boilerplate the language or framework already provides, accidental O(n²) where O(n) is one line away, verbose guards a single optional chain or default covers, type narrowing control flow already implies.
...
- ✅ Replacing a sequential await loop with the stdlib's parallel primitive, where order doesn't matter.
...
Split a file only when several cohesive groups are sharing it and each would read better alone. Follow the repo's existing layout — read sibling directories before inventing a structure.
...
- ❌ Removing `// 5000ms, from the observed p99` — that's a cited magic number.
- ❌ Removing `// the API returns 200 with an error body here` — that's an external contract.
